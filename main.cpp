...
const std::string buttonFilename = "mintycustombuttons.txt";

void saveButtonInfo() {

	std::ofstream buttonFile(buttonFilename);

	if (!buttonFile.is_open()) {
		std::cerr << "Error opening button info file for writing!" << std::endl;
		return;
	}

	for (const auto& button : buttonFuncs) {
		buttonFile << button.first << "," << button.second.target_type().name() << "," << std::endl;
	}

	buttonFile.close();
}

bool buttonInfoLoaded = false;
void loadButtonInfo() {
	if (buttonInfoLoaded) {
		return; // Button information has already been loaded
	}

	std::ifstream buttonFile(buttonFilename);

	if (!buttonFile.is_open()) {
		std::cerr << "Error opening button info file for reading!" << std::endl;
		return;
	}
	std::string functionText;
	std::string buttonLabel, buttonFunctionType, line;
	while (std::getline(buttonFile, buttonLabel, ',')) {
		std::getline(buttonFile, buttonFunctionType, ',');

		std::function<void()> buttonFunc;
		if (buttonFunctionType == typeid(buttonFunc).name()) {
			// Function type matches expected type
			std::getline(buttonFile, functionText, ',');

			buttonFunc = [functionText]() {
			...
			// Function type does not match expected type
			std::cerr << "Error: button function type mismatch for button label '" << buttonLabel << "'!" << std::endl;
		}

		std::getline(buttonFile, line); // Read the end-of-line character(s)
	}

	buttonFile.close();
	buttonInfoLoaded = true;
}
